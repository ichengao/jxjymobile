	    ...
	    <div class="main">
	        <div class="msg">
	            ...
	            <p class="m_time">
	            	<table width='100%'>
	            		<tr>
	            			<td width="50%">发布时间：{{dataMsg.publishtime}}</td>
	            			<td>来源：{{dataMsg.publishfrom}}</td>
	            		</tr>
	            	</table>
	            </p>
	            <p class="m_content"></p>
		...
		methods:{
			getData(){
				...
				.then(function(res){
					...
					_this.dataMsg.publishtime=_this.dataMsg.publishtime.split(" ")[0];

					// 取三条
					_this.dataMsg3=res.data.relatedInfoMessageList;
					var img = _this.dataMsg.infoContent.split('<img alt=""');
					for(var i=0;i<img.length-1;i++){
                        ...
                        _this.dataMsg.infoContent=_this.dataMsg.infoContent.replace('<img alt=""','<img width="100%"');
					}
					$('.m_content').append(_this.dataMsg.infoContent);
					$('.m_content img').css({'maxWidth':'100%','height':'auto'});
				});
			},
			// 页面跳转
			judgePage(e){
				...
				_this.$router.replace({path:'/helpDetail',query:{'codeNo':_this.codeNo,'name':_this.name}});
			},
			// 设置是否显示头部返回按钮
			setHeaderBackBtn(){
...
<style scoped>
    ...
    .m_content img{max-width: 100% !important;}
    ...
    .page_change table tr td:first-child{vertical-align: top;width:1.5rem;}
