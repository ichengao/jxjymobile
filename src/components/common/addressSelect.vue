      	...
      	<section class="address">
	        ...
	        <section class="title" ref="areaSelected">
	        	 <!-- @click="provinceSelected()" -->
	          	<div class="area">{{Province?Province:'省份'}}</div>
...
<script>
	...
	export default {
	  	...
	  	methods: {
	  		// 获取市级区域  安徽省默认编码34
	    	getData: function(code,input,index) {
	      		this.province = '34';
	      		this.Province = '安徽';
	     		...
	     		this.info.map( a => a.selected = false );
	      		this.info[0].selected = true;
	      		// 保存选择的省信息
	      		this.areaMsg.provice='安徽';  
	  			...
	  			.then(function(res){
	  				...
	  				var getData1={};
	  				_this.getClick.push('34');

	  				if(res.data.result == '1'){
	  					for(var i=0;i<dataMsg.length;i++){
							getData1={id:"",name:"",district:[]};
	  						getData1.id=dataMsg[i].codeid;
	  						getData1.name=dataMsg[i].cityname;
	  						_this.info[0].city.push(getData1);
	  					}
